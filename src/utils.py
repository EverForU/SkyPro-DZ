...
def read_json_file(file_path):
    ...
    try:
        ...
        if isinstance(data, list):
            utils_logger.info(f"Файл {file_path} успешно прочитан. Количество записей: {len(data)}")
            return data
        else:
            utils_logger.error(f"Ошибка: файл {file_path} содержит не список, а {type(data).__name__}")
            print("Ошибка: файл содержит не список")
            return []

    except FileNotFoundError:
        utils_logger.error(f"Ошибка: файл {file_path} не найден")
        print(f"Ошибка: файл {file_path} не найден")
        return []
    except json.JSONDecodeError as e:
        utils_logger.error(f"Ошибка: файл {file_path} содержит неверный JSON: {str(e)}")
        print(f"Ошибка: файл {file_path} содержит неверный JSON")
        return []
    except Exception as e:
        utils_logger.error(f"Непредвиденная ошибка при чтении файла {file_path}: {str(e)}")
        print(f"Ошибка: {e}")
        return []
